...
{
    //传正则表达式进来，直接进行getNFA和serializeNFA运算
...
NFA NfaMananger::getNFA(const string &reg)
{
    ...
    if(leapIds.size() != 0)
    {
        ...
        for(NFA nfa : nfas)
        {
            ...
            //进行连边的操作，这里没有完全遵守Tompson规则，如a|b|c按照三线并执行
            ...
            //进行出入边与节点相连的修正，如果处理(a|b)则会输出8个数字
        }
